...
"""Orchestrator — the pipeline controller (formerly split into BaseOrchestrator and NewOrchestrator).
...
class Orchestrator:
    def __init__(self, crawler, repo: MSSQLRepository, downloader: Downloader = None, *,
                 ...
                 change_root=None,
                 ocr_engine: HTMLFallbackEngine = None):
        ...
        # What THIS run wrote, for the review API (storage/run_store.py). Appended from
        # worker threads, hence the lock.
        self._run_changes: List[dict] = []
    ...
    def _clean_identity(identity) -> tuple:
        ...
        """
        return clean_fields(identity) or Orchestrator.DEFAULT_IDENTITY
    ...
    def check_run_trustworthy(self, docs: List) -> tuple:
        ...
        for w in run.get("warnings", []) or []:
            # "coverage gap": a source read fewer rows than the site's own list
            # holds. Untrusted, so its missing rows cannot be ruled `disappeared`
            # and the short count cannot become the next run's baseline.
            if ("capped" in w.lower() or "stopped at page" in w.lower()
                    or "coverage gap" in w.lower()):
                problems.append(w[:160])
    ...
    def _process_versioned_doc(self, doc):
        ...
        if status == "modified" and existing_id:
            ...
            regulation_id = existing_id
            self._note_change("modified", doc, regulation_id, version_id, old_version_id,
                              old_chars=len((prev.get("content_text") or prev.get("content_html") or "")),
                              new_chars=len((meta.get("content_text") or getattr(doc, "document_html", "") or "")))
            ...
            self._note_change("new", doc, regulation_id, version_id, None)
    ...
    # Class-level so an instance built without __init__ (tests do that) still has one.
    _changes_lock = threading.Lock()
    _run_changes: List[dict] = []

    def _note_change(self, kind, doc, regulation_id, version_id, previous_version_id, **detail):
        """Remember one document this run wrote, for the review API."""
        row = {"type": kind, "regulation_id": regulation_id, "version_id": version_id,
               "previous_version_id": previous_version_id,
               "title": str(getattr(doc, "title", "") or "")[:500],
               "source_system": str(getattr(doc, "source_system", "") or ""),
               "document_url": str(getattr(doc, "document_url", "") or "")[:1000],
               "detail": detail}
        with self._changes_lock:
            self._run_changes.append(row)

    def _changes_block(self, buckets, withdrawals) -> dict:
        """new / modified / deleted, as the review API stores them.

        `deleted` is the crawl's `disappeared` bucket, each row tagged with what the
        withdrawal rule made of it (proposed / watching / not-judged). Nothing here has
        been withdrawn: it is a list for a person.
        """
        verdict = {}
        for label in ("withdrawal-proposed", "watching", "not-judged"):
            for e in (withdrawals or {}).get(label) or []:
                verdict[e.get("url") or e.get("key")] = label
        deleted = []
        for row in buckets.get("disappeared") or []:
            row = row if isinstance(row, dict) else {}
            url = row.get("document_url") or ""
            deleted.append({
                "type": "deleted", "regulation_id": row.get("id"), "version_id": None,
                "previous_version_id": None,
                "title": str(row.get("title") or "")[:500],
                "source_system": str(row.get("source_system") or ""),
                "document_url": str(url)[:1000],
                "detail": {"withdrawal": verdict.get(url, "watching")}})
        with self._changes_lock:
            done = list(self._run_changes)
        return {"new": [c for c in done if c["type"] == "new"],
                "modified": [c for c in done if c["type"] == "modified"],
                "deleted": deleted}
    ...
    def run_for_regulator(self, regulator_name: str) -> Dict:
        ...
        if (last and last.get("inventory_hash") == inv
            ...
            self.report["changes"] = self._changes_block(buckets, self.report["withdrawals"])
            return self.report
        ...
        self.report["changes"] = self._changes_block(buckets, withdrawals)
        if len(groups) > 1:
    ...
    # -------------------------------------------------------------------- #
    #  FROM THE OLD BaseOrchestrator: the only members orch.py used
    # -------------------------------------------------------------------- #
